...
class GomokuGame(TwoPlayerGame):
    ...
    def convertIndexToPos(self, i):
        return (i//self.size[0], i%self.size[0])

    def convertPosToIndex(self, p):
        return p[0]*self.size[0]+p[1]
    ...
    def _threeThree(self, color, pos):

        num_three = 0
        for dir in [(0,1),(1,0),(1,1),(1,-1)]:
            in_a_row = self._checkThree(color, pos, dir)
            if in_a_row == 3:
                num_three += 1
            elif in_a_row == 4:
                return False
    ...
    def _checkThree(self, color, pos, dir):
        num_row = 1
        num_row_blank = 1
        ...
        for d in [-1, 1]:
            for i in range(1, 4):
                next = (pos[0] + dir[0] * i * d, pos[1] + dir[1] * i * d)
                if (next[0] < 0 or next[0] >= self.size[0] or next[1] < 0 or next[1] >= self.size[1]):
                    break

                if self.b[next] == -color:
                    break
                elif self.b[next] == color:
                    if blank == 0:
                        num_row += 1
                    num_row_blank += 1
                else:
            ...
            if num_row > 3 or num_row_blank > 3:
                return 4
    ...
    def isLegal(self, c, pos):
        ...
        # return self.b[pos] == self.EMPTY and not self._threeThree(c, pos)
